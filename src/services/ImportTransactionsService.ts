...
class ImportTransactionsService {
  async execute(filePath: string): Promise<Transaction[]> {
    ...
    const parsers = csvParse({
      from_line: 2, // Linha que vai começar a ler
    });

    const parseCSV = constactsReadStream.pipe(parsers); // Vai lendo linha por linha do arquivo
    ...
    parseCSV.on('data', async line => {
      const [title, type, value, category] = line.map(
        (cell: string) => cell.trim(), // Retira os espaços
      );
    ...
    // Final das validações das categorias
